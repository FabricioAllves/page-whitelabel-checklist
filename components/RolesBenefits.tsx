    ...
    <section className="section-contrast py-24">
      <div className="shell px-4 sm:px-6">
            ...
            <button
              ...
              className={`rounded-full border px-5 py-3 text-sm font-semibold ${
                activeRole === role.id
                  ...
                  : "border-white/10 bg-white/5 text-[#9dafcb] hover:text-white"
              }`}
              style={
                activeRole === role.id
                  ? { borderColor: `${role.color}55`, boxShadow: `0 0 0 1px ${role.color}25 inset` }
                  : {}
              }
            >
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={activeRole}
            className="panel-dark mx-auto max-w-4xl overflow-hidden rounded-[34px]"
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            <div
              ...
              style={{
                background: `linear-gradient(135deg, ${current.color}18 0%, rgba(255,255,255,0.03) 60%)`,
              }}
            >
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  ...
                  <h3 className="mt-3 text-3xl font-semibold text-white">{current.title}</h3>
                  <p className="mt-2 max-w-xl text-base leading-7 text-[#a8b7d0]">{current.tagline}</p>
                </div>
                <div
                  className="flex h-[4.5rem] w-[4.5rem] items-center justify-center rounded-[28px] border border-white/10 text-4xl"
            ...
            <div className="grid gap-4 p-8 sm:grid-cols-2">
              {current.benefits.map((benefit, i) => (
                <motion.div
                  key={benefit}
                  className="rounded-[24px] border border-white/[0.08] bg-white/5 px-5 py-4"
                  initial={{ opacity: 0, x: -10 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.25, delay: i * 0.05 }}
                >
                  <div className="mb-3 h-1 w-14 rounded-full" style={{ background: current.color }} />
                  <span className="text-sm leading-7 text-[#d9e4f2]">{benefit}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
